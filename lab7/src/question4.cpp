...
int do_job(long id, string server) {
    ...
    if (((*((uint32_t*)ent->h_addr_list[0])) & htonl(0xff000000)) == htonl(0x0a000000) || ((*((uint32_t*)ent->h_addr_list[0])) & htonl(0xff000000)) == htonl(0x7f000000) || ((*((uint32_t*)ent->h_addr_list[0])) & htonl(0xfff00000)) == htonl(0xac100000) || ((*((uint32_t*)ent->h_addr_list[0])) & htonl(0xffff0000)) == htonl(0xc0a80000))
...
int pow() {
    ...
    if (SHA1((unsigned char*)buf, len + olen, cksum) == NULL)
        return -1;
    for (i = 0; i < 20; i++)
        snprintf(&buf[i * 2], 4, "%2.2x", cksum[i]);
    printf("SHA1: %s\n\n", buf);
...
int main() {
    ...
    while ((c = menu()) != 'q') {
        ...
        case 'r':
            ...
            if (fgets(buf, sizeof(buf), stdin) != NULL && (ptr = strtok_r(buf, " \t\n\r", &saveptr)) != NULL) {
                add_job(ptr);
